            ...
            <div key={idx} className={`bg-white border border-black/5 rounded-3xl p-8 shadow-[0_20px_40px_rgb(0,0,0,0.06)] flex flex-col ${idx === 1 ? 'ring-2 ring-indigo-500 ring-offset-8 ring-offset-white rounded-3xl' : ''}`}>
              ...
              <div className="flex flex-col gap-2 mb-8 border-l-4 border-[#6366F1] pl-4">
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest mb-1 pb-1 text-black/50">/ IMG (Simple)</span>
                </div>
                <div className="flex items-end gap-2">
                  <span className="text-lg font-black font-outfit text-black/60">{bundle.priceComplex}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-black/40">/ IMG (Complex)</span>
                </div>
              </div>
              
              <ul className="mb-8 space-y-3 flex-grow">
                {bundle.features.map((f, i) => (
                  <li key={i} className="flex gap-2 items-start text-sm font-medium text-black/80">
