    ...
    <div className="min-h-screen bg-[#F9F9F9] flex flex-col font-inter">
      ...
      <main className="flex-grow py-12 px-10 md:px-20 lg:px-[10%] xl:px-[12%] pb-32">
        ...
        <div className="flex flex-col gap-8 max-w-4xl">
          <div>
            <label className="block text-[12px] font-bold uppercase tracking-widest mb-2">Job Name</label>
          ...
          <div>
            <label className="block text-[12px] font-bold uppercase tracking-widest mb-4">Select Turnaround Speed (v2.1 Metric)</label>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
               {['12h (Rush)', '24h', '48h', '72h', '7 Days'].map((t, idx) => (
                 <label key={idx} className={`border-2 p-4 cursor-pointer hover:border-[#6366F1] transition-colors ${idx === 1 ? 'border-black bg-black/5' : 'border-black/20 bg-white'}`}>
                   ...
                   <span className="block text-center font-bold uppercase tracking-tight text-[15px]">{t}</span>
                 </label>
               ))}
            </div>
          </div>
          ...
          <div>
            <label className="block text-[12px] font-bold uppercase tracking-widest mb-2">Upload Images</label>
            <div className="border-2 border-dashed border-black/30 rounded-3xl bg-white p-20 flex flex-col items-center justify-center text-center cursor-pointer hover:border-[#6366F1] hover:bg-[#6366F1]/5 transition-all">
              ...
              <p className="text-[15px] font-medium text-black/60">Upload directly or link Google Drive / Dropbox.</p>
              <p className="text-[12px] font-bold uppercase tracking-widest text-black/40 mt-4">JPG, PNG, TIFF — Max 50MB per image</p>
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 w-full bg-[#121212] border-t-4 border-[#6366F1] py-4 px-10 md:px-20 lg:px-[10%] flex justify-between items-center z-50">
          <div className="text-white">
            ...
            <span className="text-[12px] uppercase tracking-widest text-[#6366F1] font-bold">Total updates based on Turnaround</span>
          </div>
          <a href="/jobs/new/review" className="bg-[#6366F1] text-white px-8 py-4 rounded-full text-[13.5px] font-bold tracking-widest uppercase hover:bg-white hover:text-black transition-colors">Review & Submit —</a>
        </div>
      </main>
    </div>
  );
