      ...
      <main className="flex-grow py-12 px-10 md:px-20 lg:px-[10%] xl:px-[12%] pb-32">
        ...
        <div className="flex flex-col gap-8 max-w-4xl">
          <div>
            ...
            <input type="text" placeholder="E.g., Spring Collection 2026" className="w-full p-4 border border-black/5 rounded-3xl outline-none font-medium focus:border-[#6366F1] transition-colors" />
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
               {['12h (Rush)', '24h', '48h', '72h', '7 Days'].map((t, idx) => (
                 <label key={idx} className={`border-2 p-4 cursor-pointer hover:border-[#6366F1] transition-colors ${idx === 1 ? 'border-black bg-gray-50/80 rounded-2xl' : 'border-black/20 bg-white'}`}>
          ...
          <div>
            ...
            <div className="border border-dashed border-black/15 rounded-3xl bg-white p-20 flex flex-col items-center justify-center text-center cursor-pointer hover:border-[#6366F1] hover:bg-[#6366F1]/5 transition-all">
              <div className="w-16 h-16 bg-black text-white rounded-full shadow-lg shadow-black/10 flex items-center justify-center mb-6">
        ...
        <div className="fixed bottom-0 left-0 w-full bg-[#121212] border-t-4 border-[#6366F1] py-4 px-10 md:px-20 lg:px-[10%] flex justify-between items-center z-50">
          ...
          <a href="/jobs/new/review" className="bg-[#6366F1] text-white px-8 py-4 text-[11px] rounded-full font-bold tracking-widest uppercase hover:bg-white hover:text-black transition-colors">Review & Submit —</a>
        </div>
      </main>
    </div>
  );
